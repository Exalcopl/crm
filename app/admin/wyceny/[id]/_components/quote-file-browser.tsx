...
function FolderNode({
  ...
  const loadContents = useCallback(async () => {
    ...
    try {
      const result = await actions.list({ quoteId, folderId: item.id });
      setContents(result);
    } catch (e) {
  ...
  async function handleDeleteChild(childId: string) {
    ...
    try {
      await actions.deleteItem({ quoteId, itemId: childId });
      await loadContents();
    } catch (e) {
  ...
  async function handleDeleteSelf() {
    ...
    try {
      await actions.deleteItem({ quoteId, itemId: item.id });
      onDeleteSelf();
    } catch (e) {
...
export function QuoteFileBrowser({
  quote,
  archived,
}: {
  quote: QuoteShape;
  ...
  const listContents = useAction(api.sharepoint.listFolderContents);
  const uploadSession = useAction(api.sharepoint.createUploadSessionInFolder);
  const createFolder = useAction(api.sharepoint.createSharepointSubfolder);
  const deleteItem = useAction(api.sharepoint.deleteSharepointItem);
  ...
  const loadRoot = useCallback(async () => {
    ...
    setRootLoading(true);
    try {
      const result = await listContents({ quoteId: quote._id });
      setRootItems(result);
    } catch (e) {
  ...
  }, [quote._id, hasFolder, listContents]);
  ...
  async function handleDeleteRootFile(itemId: string) {
    ...
    try {
      await deleteItem({ quoteId: quote._id, itemId });
      await loadRoot();
    } catch (e) {
