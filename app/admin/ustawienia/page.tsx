...
export default function UstawieniaPage() {
  ...
  // OCR settings
  const currentProvider = useQuery(api.systemSettings.getOcrProvider);
  ...
  if (isLoading) {
    return (
      <main className="fluent-content" style={{ display: "flex", justifyContent: "center", padding: "100px", color: "var(--text-muted)" }}>
  ...
  if (!isAdmin) {
    return (
      <main className="fluent-content" style={{ padding: "40px", color: "var(--text-muted)", textAlign: "center" }}>
  ...
  async function handleSaveOcr() {
    ...
    try {
      ...
      setOcrMessage({ text: "Zapisano ustawienia dostawcy OCR AI.", type: "success" });
      toast.success(`Przełączono dostawcę AI OCR na ${selectedProvider === "anthropic" ? "Anthropic Claude" : "Google Gemini"}`);
    } catch (e: any) {
      const errMsg = e.message || "Błąd zapisu ustawień AI.";
      setOcrMessage({ text: errMsg, type: "error" });
      toast.error(errMsg);
    } finally {
      setSavingOcr(false);
    }
  ...
  async function handleSaveNotif() {
    ...
    try {
      ...
      const errMsg = e.message || "Błąd zapisu powiadomień.";
      setNotifMessage({ text: errMsg, type: "error" });
      toast.error(errMsg);
    } finally {
  ...
  return (
    <main className="fluent-content" style={{ padding: "24px", overflowY: "auto" }}>
      <div style={{ maxWidth: "680px", margin: "0 auto", display: "flex", flexDirection: "column", gap: "24px", paddingBottom: "40px" }}>
        ...
        >
          <div>
            <h2 style={{ fontSize: "16px", fontWeight: 600, margin: 0, color: "var(--text-secondary)" }}>
              🤖 Konfiguracja Silnika AI OCR (Rozpoznawanie Dokumentów)
            </h2>
            <p style={{ fontSize: "13px", color: "var(--text-muted)", marginTop: "4px", marginBottom: 0, lineHeight: 1.5 }}>
              Wybierz aktywnego dostawcę silnika AI/LLM do analizowania i wyciągania tabel z plików ofert, wycen i dokumentów RW.
            </p>
          </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
            ...
            <div
              ...
              style={{
                ...
                padding: "14px",
                borderRadius: "8px",
                background: selectedProvider === "anthropic" ? "rgba(212, 29, 60, 0.08)" : "rgba(255, 255, 255, 0.02)",
                border: selectedProvider === "anthropic" ? "1.5px solid #d41d3c" : "1px solid rgba(255, 255, 255, 0.05)",
                cursor: "pointer",
            ...
            >
              <input
                ...
                style={{ accentColor: "#d41d3c", marginTop: "3px", cursor: "pointer" }}
              />
              <div style={{ flex: 1 }}>
                <div style={{ fontWeight: 600, fontSize: "14px", color: "var(--text-primary)", display: "flex", alignItems: "center", gap: "8px" }}>
                  <span>Anthropic Claude API (Sonnet 3.5 / Haiku)</span>
                  {selectedProvider === "anthropic" && (
                    <span style={{ fontSize: "10px", background: "#d41d3c", color: "#fff", padding: "2px 6px", borderRadius: "4px", fontWeight: 700 }}>AKTYWNY</span>
                  )}
                </div>
                <div style={{ fontSize: "12px", color: "var(--text-muted)", marginTop: "4px" }}>
                  Wysoka precyzja analizy dokumentów tabelarycznych i skanów. Wymaga klucza <code style={{ color: "#d41d3c" }}>ANTHROPIC_API_KEY</code> w środowisku Convex.
                </div>
              </div>
            </div>
            ...
            <div
              ...
              style={{
                ...
                padding: "14px",
                borderRadius: "8px",
                background: selectedProvider === "gemini" ? "rgba(59, 130, 246, 0.08)" : "rgba(255, 255, 255, 0.02)",
                border: selectedProvider === "gemini" ? "1.5px solid #3b82f6" : "1px solid rgba(255, 255, 255, 0.05)",
                cursor: "pointer",
                transition: "all 150ms ease",
              }}
            >
              <input
                ...
                style={{ accentColor: "#3b82f6", marginTop: "3px", cursor: "pointer" }}
              />
              <div style={{ flex: 1 }}>
                <div style={{ fontWeight: 600, fontSize: "14px", color: "var(--text-primary)", display: "flex", alignItems: "center", gap: "8px" }}>
                  <span>Google Gemini API (AI Studio - Free Tier)</span>
                  {selectedProvider === "gemini" && (
                    <span style={{ fontSize: "10px", background: "#3b82f6", color: "#fff", padding: "2px 6px", borderRadius: "4px", fontWeight: 700 }}>AKTYWNY</span>
                  )}
                </div>
                <div style={{ fontSize: "12px", color: "var(--text-muted)", marginTop: "4px" }}>
                  Wykorzystuje model <code style={{ color: "#3b82f6" }}>gemini-1.5-flash</code> z darmowym limitem. Wymaga klucza <code style={{ color: "#3b82f6" }}>GEMINI_API_KEY</code> w środowisku Convex.
                </div>
              </div>
            </div>
          </div>
